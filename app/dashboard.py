...
==========================================
Dashboard analítico do projeto IBGE Lakehouse.
...
with col1:
    ...
    <div class="metric-card">
        ...
        <div class="metric-sub">{total_estados} estados</div>
    </div>""", unsafe_allow_html=True)

with col2:
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-label">PIB Brasil (estimado)</div>
        <div class="metric-value">R$ {pib_brasil:.1f} bi</div>
        <div class="metric-sub">em R$ milhões</div>
    </div>""", unsafe_allow_html=True)

with col3:
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-label">Maior PIB total</div>
        <div class="metric-value">{maior_pib_estado}</div>
        <div class="metric-sub">estado com maior economia</div>
    </div>""", unsafe_allow_html=True)

with col4:
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-label">Maior PIB per capita</div>
        <div class="metric-value">{maior_pc_estado}</div>
        <div class="metric-sub">média dos municípios</div>
    </div>""", unsafe_allow_html=True)
...
with col_a:
    fig1 = px.bar(
        df_estados.head(10),
        x="pib_total_mil_reais",
        y="estado_sigla",
        orientation="h",
        title="PIB Total por Estado (Top 10)",
        labels={"pib_total_mil_reais": "PIB (R$ mil)", "estado_sigla": "Estado"},
        color="pib_total_mil_reais",
...
with col_b:
    ...
    fig2 = px.bar(
        ...
        y="estado_sigla",
        orientation="h",
        title="PIB per Capita Médio por Estado (Top 10)",
        labels={"pib_per_capita_medio": "PIB per capita (R$)", "estado_sigla": "Estado"},
        color="pib_per_capita_medio",
...
with col_c:
    fig3 = px.bar(
        df_mun_filtrado.sort_values("pib_per_capita"),
        x="pib_per_capita",
        y="municipio_nome",
        orientation="h",
        title=titulo_mun + " · PIB per Capita",
        labels={"pib_per_capita": "PIB per capita (R$)", "municipio_nome": ""},
...
with col_d:
    ...
    st.dataframe(
        df_mun_filtrado[[
            "ranking_nacional", "municipio_nome", "estado_sigla",
            "populacao_estimada", "pib_per_capita"
        ]].rename(columns={
...
# ─── Seção 3: Dispersão PIB vs População ─────────────────────────────────────
...
fig4 = px.scatter(
    ...
    hover_data={"estado_sigla": True, "pib_per_capita": ":,.0f"},
...
with col_e:
    fig5 = px.bar(
        df_ipca,
        x=df_ipca["periodo"].astype(str),
        y="variacao_pct",
        title="Variação IPCA por Período (%)",
        labels={"x": "Período", "variacao_pct": "Variação (%)"},
        color="variacao_pct",
...
with col_f:
    fig6 = px.line(
        df_ipca,
        x=df_ipca["periodo"].astype(str),
        y="variacao_acumulada_pct",
        title="Variação Acumulada IPCA (%)",
        labels={"x": "Período", "variacao_acumulada_pct": "Acumulado (%)"},
        markers=True,
    )
